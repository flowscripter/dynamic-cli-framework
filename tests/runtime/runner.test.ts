...
describe("runner", () => {
  ...
  it("Global modifier and sub-command run", async () => {
    let modifierHasRun = false;
    let subHasRun = false;
  ...
  it("Global modifier and global run", async () => {
    let modifierHasRun = false;
  ...
  it("Two global modifier and global run", async () => {
  ...
  it("Two global modifier and group command run", async () => {
    let modifier1HasRun = false;
    let modifier2HasRun = false;
    let groupHasRun = false;
    let subHasRun = false;
  ...
  it("Global modifier and default run", async () => {
    let modifierHasRun = false;
  ...
  it("Error thrown in non-global run", async () => {
    const option = {
      name: "foo",
      type: ArgumentValueTypeName.STRING,
      shortAlias: "f",
  ...
  it("Error thrown in global run", async () => {
    const buffer = new Buffer();
    const globalCommand = getGlobalCommandWithShortAlias("global", "g", {
      type: ArgumentValueTypeName.STRING,
    });
  ...
  it("Error thrown in global modifier run", async () => {
    const buffer = new Buffer();
  ...
  it("Parse error warning in non-global run", async () => {
    const buffer = new Buffer();
    let hasRun = false;
    const option = {
      name: "foo",
  ...
  it("Missing Group Command member", async () => {
    const subCommand = getSubCommand("command", [], []);
    const groupCommand = getGroupCommand("group", [subCommand]);
  ...
  it("Unknown arg warning in non-global run", async () => {
    const buffer = new Buffer();
    let hasRun = false;
    const subCommand = getSubCommand("command", [], []);
  ...
  it("Unknown arg warning in global run", async () => {
    const buffer = new Buffer();
    let hasRun = false;
  ...
  it("Error thrown for default run", async () => {
    const buffer = new Buffer();
    const globalCommand = getGlobalCommandWithShortAlias("global", "g");
  ...
  it("Run default based on all args", async () => {
  ...
  it("Sub-Command run with complex options and explicit arrays", async () => {
    let hasRun = false;
    const options = [{
      name: "alpha",
      shortAlias: "a",
  ...
  it("Sub-Command run with complex options and positionals", async () => {
    let hasRun = false;
    const options = [{
      name: "alpha",
      shortAlias: "a",
  ...
  it("Group command as default command is invalid", async () => {
    const buffer = new Buffer();
  ...
  it("Global modifier specified fully by config and global run", async () => {
    let modifierHasRun = false;
    let globalHasRun = false;
