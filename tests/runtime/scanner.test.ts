...
describe("scanner Tests", () => {
  ...
  test("Unused leading args whilst scanning for non-modifier command", () => {
    const globalCommand = getGlobalCommandWithShortAlias("globalCommand", "g");
    const subCommand = getSubCommandWithOptionAndPositional();
  ...
  test("Sub-command with complex options scanned", () => {
  ...
  test("Global command normalising (tricky scenario 1)", () => {
    const globalCommand = getGlobalCommandWithShortAlias("globalCommand", "g");
  ...
  test("Global command normalising (tricky scenario 2)", () => {
    const globalCommand = getGlobalCommandWithShortAlias("globalCommand", "g");
  ...
  test("Two sub-commands specified (illegal scenario)", () => {
    const subCommand = getSubCommandWithOptionAndPositional();
    const nonModifierCommandsByName: ReadonlyMap<string, Command> = new Map([
      [subCommand.name, subCommand],
    ]);
  ...
  test("Global command and sub-command scanned (illegal scenario)", () => {
    const globalCommand = getGlobalCommandWithShortAlias("globalCommand", "g");
    const subCommand = getSubCommandWithOptionAndPositional();
    const nonModifierCommandsByName: ReadonlyMap<string, Command> = new Map([
      [subCommand.name, subCommand as Command],
  ...
  test("Group command and sub-command scanned (illegal scenario)", () => {
    const subCommand = getSubCommandWithOptionAndPositional();
    const groupCommand = getGroupCommand("group", [subCommand]);
    const nonModifierCommandsByName: ReadonlyMap<string, Command> = new Map([
      [subCommand.name, subCommand as Command],
