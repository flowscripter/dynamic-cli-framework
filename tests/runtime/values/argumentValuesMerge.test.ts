...
import argumentValuesMerge from "../../../src/runtime/values/argumentValuesMerge.ts";
import { assertEquals } from "../../test_deps.ts";

describe("argumentValuesMerge", () => {
  it("One layer merge", () => {
    let result = argumentValuesMerge({ a: 1 }, { b: 2 });
    assertEquals(result, { a: 1, b: 2 });

    result = argumentValuesMerge({ a: "foo" }, { b: "bar" });
    assertEquals(result, { a: "foo", b: "bar" });

    result = argumentValuesMerge({ a: true }, { b: false });
    assertEquals(result, { a: true, b: false });
  });

  it("One layer merge and override", () => {
    let result = argumentValuesMerge({ a: 1 }, { a: 2 });
    assertEquals(result, { a: 1 });

    result = argumentValuesMerge({ a: "foo" }, { a: "bar" });
    assertEquals(result, { a: "foo" });

    result = argumentValuesMerge({ a: true }, { a: false });
    assertEquals(result, { a: true });
  });

  it("One layer merge with no defaults", () => {
    let result = argumentValuesMerge({ a: 1 }, {});
    assertEquals(result, { a: 1 });

    result = argumentValuesMerge({ a: "foo" }, {});
    assertEquals(result, { a: "foo" });

    result = argumentValuesMerge({ a: true }, {});
    assertEquals(result, { a: true });
  });
  ...
  it("Override from configured number to parsed string", () => {
    const result = argumentValuesMerge({ a: "1" }, { a: 2 });
    assertEquals(result, { a: "1" });
  });

  it("Override from configured boolean to parsed string", () => {
    const result = argumentValuesMerge({ a: "true" }, { a: false });
    assertEquals(result, { a: "true" });
  });

  it("Override from configured single value to parsed array", () => {
    const result = argumentValuesMerge({ a: [1, 2] }, { a: 2 });
  ...
  it("Override configured array value with parsed single value", () => {
    const result = argumentValuesMerge({ a: 3 }, { a: [1, 2] });
  ...
  it("Complex object and arrays merge", () => {
    const result = argumentValuesMerge({ a: { b: { c: "foo" } } }, {
      d: [{ e: [{ f: 1 }] }],
    });
  ...
  it("Complex object and arrays merge and override", () => {
    let result = argumentValuesMerge({
  ...
  it("Primitive array merge and insert", () => {
    let result = argumentValuesMerge({ a: [1, 2, 3] }, { a: [1, 2, 3] });
    assertEquals(result, { a: [1, 2, 3] });

    result = argumentValuesMerge({ a: [1, 2, 3, 4] }, { a: [1, 2, 3] });
    assertEquals(result, { a: [1, 2, 3, 4] });

    result = argumentValuesMerge({ a: [1, 2, 3] }, { a: [1, 2, 3, 4] });
    assertEquals(result, { a: [1, 2, 3, 4] });

    result = argumentValuesMerge({ a: [1, 2, 3] }, { a: [4, 5, 6, 7] });
    assertEquals(result, { a: [1, 2, 3, 7] });
  });

  it("Complex object array merge and insert", () => {
    let result = argumentValuesMerge({ a: [{ b: 1 }, { c: 2 }, { d: 3 }] }, {
      a: [{ b: 1 }, { c: 2 }, { d: 3 }],
    });
    ...
    result = argumentValuesMerge({ a: [{ b: 1 }, { c: 2 }, { d: 3 }] }, {
      a: [{ b: 1 }, { c: 2 }, { d: 3 }, { e: 4 }],
    });
    assertEquals(result, { a: [{ b: 1 }, { c: 2 }, { d: 3 }, { e: 4 }] });
    ...
    result = argumentValuesMerge({ a: [{ a: 1 }, { a: 2 }, { a: 3 }] }, {
      a: [{ a: 4 }, { a: 5 }, { a: 6 }, { a: 7 }],
    });
    assertEquals(result, {
      a: [{ a: 1 }, { a: 2 }, { a: 3 }, { a: 7 }],
  ...
  it("Complex object and arrays merge with no defaults", () => {
    const result = argumentValuesMerge({
      a: { b: { c: "foo" } },
      d: [{ e: [{ f: 1 }] }],
    }, {});
    assertEquals(result, { a: { b: { c: "foo" } }, d: [{ e: [{ f: 1 }] }] });
  ...
  it("One layer merge with undefined values", () => {
    let result = argumentValuesMerge({ a: undefined }, { b: 1 });
    assertEquals(result, { a: undefined, b: 1 });

    result = argumentValuesMerge({ a: undefined }, { b: "foo" });
    assertEquals(result, { a: undefined, b: "foo" });

    result = argumentValuesMerge({ a: undefined }, { b: true });
    assertEquals(result, { a: undefined, b: true });
  });

  it("One layer merge and override with undefined values", () => {
    let result = argumentValuesMerge({ a: undefined }, { a: 1 });
    assertEquals(result, { a: 1 });

    result = argumentValuesMerge({ a: undefined }, { a: "foo" });
    assertEquals(result, { a: "foo" });

    result = argumentValuesMerge({ a: undefined }, { a: true });
    assertEquals(result, { a: true });
  });

  it("Primitive array merge with undefined values", () => {
    const result = argumentValuesMerge({ a: [1, 2, 3, undefined] }, {
      a: [1, 2, 3],
    });
  ...
  it("Primitive array merge and override with undefined values", () => {
    const result = argumentValuesMerge({ a: [1, 2, undefined] }, {
      a: [1, 2, 3],
    });
    assertEquals(result, { a: [1, 2, 3] });
  });
  ...
  it("Complex object merge with undefined objects", () => {
    const result = argumentValuesMerge({ a: { b1: undefined } }, {
      a: { b2: { c: "foo" } },
    });
  ...
  it("Complex object merge and override with undefined objects", () => {
    const result = argumentValuesMerge({ a: { b: undefined } }, {
      a: { b: { c: "foo" } },
    });
