...
function getCommandString(printerService: PrinterService, parseResult: ParseResult): string {
  ...
  if (groupCommand) {
    return printerService.yellow(`'${groupCommand.name}:${command.name}'`);
  }
...
export async function printParseResultError(
  ...
  const commandErrorString = isDefaultCommand
    ? "Parse error: "
    : `Parse error for command: ${getCommandString(printerService, parseResult)}`;
  const { command, invalidArguments } = parseResult;

  let errorString = "=> ";

  const skipArgName = isGlobalModifierCommand(command) || isGlobalCommand(command);
  const argsString = invalidArguments
    .map((arg) => getInvalidArgumentString(arg, skipArgName))
    .join(", ");
  errorString = `${errorString}${printerService.yellow(argsString)}`;

  await printerService.error(`${commandErrorString}\n  ${errorString}\n\n`, Icon.FAILURE);
}
...
export async function printCommandExecutionError(
  ...
  const commandErrorString = isDefaultCommand
    ? "Execution error: "
    : `Execution error for command: ${getCommandString(printerService, parseResult)}`;
  if (err !== undefined) {
...
export async function printNoCommandRecognisedError(
  ...
  if (availableArgs.length > 0) {
    ...
    for (const arg of availableArgs) {
      ...
      const possibleCommandNames = findPossibleCommandNames(arg, groupCommands, subCommands);
...
export async function printUnusedArgsWarning(
  ...
  if (overallUnusedArgs.length === 1) {
    await printerService.warn(`Unused arg: ${overallUnusedArgs[0]}\n\n`, Icon.ALERT);
  } else {
    await printerService.warn(`Unused args: ${overallUnusedArgs.join(" ")}\n\n`, Icon.ALERT);
  }
}
