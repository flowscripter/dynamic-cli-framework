...
abstract class MultiCommandCliAbstractHelpCommand {
  ...
  constructor(cliConfig: CLIConfig, commandRegistry: CommandRegistry) {
  ...
  private async printGenericHelp(
    context: Context,
    ...
    await printHelpSections(stdoutPrinter, helpSections);
  }

  private async printUsageHelp(
    context: Context,
    commandName: string,
    printer: Printer,
  ): Promise<void> {
    ...
    if (subCommand === undefined) {
      const result = this.commandRegistry
        .getGroupCommandAndMemberSubCommandByName(commandName);
      if (result !== undefined) {
        groupCommand = result.groupCommand;
        subCommand = result.memberSubCommand;
      }
    }

    if (subCommand === undefined) {
      await printer.print(
        `Unknown command: ${printer.red(commandName)}\n`,
        Icon.FAILURE,
      );
      ...
      await this.printGenericHelp(context, printer);
      return;
    }
    ...
    helpSections.push(
      ...getCommandArgsHelpSections(this.cliConfig, subCommand, false),
    );

    helpSections.push(
      ...getCommandExamplesHelpSections(printer, context, subCommand),
    );
  ...
  public async execute(
    ...
    const commandName = (argumentValues["command"] || argumentValues["help"]) as
      | string
      | undefined;
    if (commandName !== undefined) {
      await this.printUsageHelp(context, commandName, printer);
    } else {
      await this.printGenericHelp(context, printer);
    }
  }
